...
def getdatax():
    try:
        ...
        posts = mongo.db.postdata.find({"userid": userid}).sort("created", -1)
        post_list = []
        for post in posts:
            post_list.append({
                "userid": post['userid'],
...
def get_user_data():
    userid = '1'
    posts1 = mongo.db.homedata.find({"userid": userid}).sort("created", -1)
    post_list1 = []
    for post in posts1:
        post_list1.append({
            "id": post['id'],
            "userid": post['userid'],
            "titlefirst": post['titlefirst'],
            "titlesecond": post['titlesecond'],
            "linkurlcv": post['linkurlcv'],
            "logourl": post['logourl']
        })

    posts2 = mongo.db.homedataimage.find({"userid": userid}).sort("created", -1)
    post_list2 = []
    ...
    for post in posts2:
        post_list2.append({
            ...
            "linkurlmedia": post['linkurlmedia']
        })


    alldata={'profile':post_list1,'media':post_list2}
    return jsonify(alldata)     
...
def update_selected():
    try:
        data = request.get_json(force=True)
        post_ids = data['settleList']
        userid = str(data['userid'])

        mongo.db.postdata.update_many({"userid": userid}, {"$set": {"selected": "false"}})

        if post_ids:
            mongo.db.postdata.update_many(
                {"postid": {"$in": post_ids}, "userid": userid},
                {"$set": {"selected": "true"}}
            )
        return jsonify({'status': 200})
    except Exception as e:
        logger.error(f"Update selected error: {e}")
        return jsonify({'status': 500, 'error': str(e)}), 500
...
def delete_item():
    try:
        ...
        post = mongo.db.postdata.find_one({"postid": post_id, "userid": userid})
        if not post:
            return jsonify({'error': 'Post not found'}), 404

        if post.get('imageurl'):
            public_id = "Uploads/" + os.path.splitext(os.path.basename(post['imageurl']))[0]
            try:
                cloudinary.uploader.destroy(public_id)
            except Exception as e:
                logger.warning(f"Cloudinary deletion warning: {e}")

        mongo.db.postdata.delete_one({"postid": post_id, "userid": userid})
        return jsonify({'status': 200, 'message': 'Deleted successfully'})
    except Exception as e:
